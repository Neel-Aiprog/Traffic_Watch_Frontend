...
export default function AccessGate({ onAccessGranted, isRegisterMode = false, onAuthModeToggle }) {
  ...
  return (
    <div className="relative min-h-screen w-full flex items-center justify-center overflow-hidden bg-primary-container grid-animated"
      ...
      {/* HUD top-left — typewriter reveal */}
      <div className="fixed top-0 left-0 p-4 pointer-events-none z-20 font-data-md text-[10px] text-outline/70 leading-relaxed tracking-widest">
      ...
      {/* HUD top-right */}
      <div className="fixed top-0 right-0 p-4 pointer-events-none z-20 text-right font-data-md text-[10px] text-outline/70 leading-relaxed tracking-widest">
      ...
      {/* Corner HUD brackets — full screen */}
      {[
        "fixed top-16 left-8 border-t border-l",
        "fixed top-16 right-8 border-t border-r",
        "fixed bottom-8 left-8 border-b border-l",
        "fixed bottom-8 right-8 border-b border-r",
      ].map((cls, i) => (
        <div key={i} className={`${cls} w-10 h-10 border-outline/20 bracket-animated pointer-events-none z-10`}
          style={{ animationDelay: `${i * 0.15}s` }} />
      ))}

      {/* Login card */}
      <div className={`relative z-10 w-full max-w-sm mx-4 ${mounted ? "card-mount" : "opacity-0"}`}>
        ...
        {[
          ...
          <div key={i} className={`${cls} w-6 h-6 border-primary`}
            style={{ animationDelay: `${0.5 + i * 0.1}s` }} />
        ))}

        <div className={`bg-surface-container-low border border-outline-variant p-8 flex flex-col gap-5 ${shake ? "animate-shake" : ""}`}
          style={{ backdropFilter: "blur(2px)" }}>

          {/* Header */}
          <div className="text-center border-b border-outline-variant pb-5 space-y-2">
          ...
          <form onSubmit={handleVerify} className="space-y-4">
            <div>
              <label className="block font-label-caps text-[10px] text-outline tracking-widest mb-1.5">ACCESS CODE</label>
            ...
            <div>
              <label className="block font-label-caps text-[10px] text-outline tracking-widest mb-1.5">PASSWORD</label>
              <div className="relative">
                ...
                <button type="button" onClick={() => setShowPassword(!showPassword)}
                  className="absolute right-3 top-1/2 -translate-y-1/2 font-label-caps text-[9px] text-outline/60 hover:text-outline tracking-widest transition-colors">
            ...
            {isRegisterMode && (
              <>
                <div>
                  <label className="block font-label-caps text-[10px] text-outline tracking-widest mb-1.5">CONFIRM CODE</label>
                ...
                <div>
                  <label className="block font-label-caps text-[10px] text-outline tracking-widest mb-1.5">CLEARANCE LEVEL</label>
                  <select className={inputBase} value={role} onChange={e => setRole(e.target.value)} disabled={isVerifying}>
            ...
            {errorMessage && (
              <div className="flex items-center gap-2 bg-error-container/20 border border-error/30 px-3 py-2.5 font-data-md text-[11px] text-error tracking-widest animate-shake">
            ...
            <button type="submit" disabled={isVerifying}
              className="btn-scan w-full mt-1 bg-surface-container border border-outline-variant text-primary font-label-caps text-[10px] tracking-[0.22em] py-4 hover:border-primary hover:bg-surface-container-high active:scale-[0.98] transition-all flex items-center justify-center gap-3 disabled:opacity-50">
          ...
          <button onClick={onAuthModeToggle}
            className="text-center font-label-caps text-[9px] text-outline/50 hover:text-outline/80 tracking-widest transition-colors underline underline-offset-2 decoration-outline/20">
            {isRegisterMode
